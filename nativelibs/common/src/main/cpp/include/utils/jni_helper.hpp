...
namespace lsplant {
    ...
    /// PushLocalFrame 与 PopLocalFrame 函数对的包装
    /// 在局部代码快中使用PushLocalFrame设置局部变量数列，在这个空间中声明的局部变量会在PopLocalFrame时销毁
    class JNIScopeFrame {
        JNIEnv *env_;
    ...
    /// decay_t<T> 返回T的类型
    /// is_convertible_v<From, To> 将数据类型From隐式转换成To，返回bool true 转换成功 false 转换失败
    /// 如果是ScopedLocalRef类型则取出其BaseType
    /// 限制泛型类型T必须是U的可转换类型
    ...
    [[maybe_unused]] inline auto JNI_SafeInvoke(JNIEnv *env, Func JNIEnv::*f, Args &&...args) {
        ...
        // invoke_result_t 可以返回任何可调用的返回值类型
